...
import { cn } from "@/lib/utils"; // Integrated cn utility
import {
  Search,
  Plus,
  Edit2,
    ...
    <div className={cn("min-h-screen p-8 transition-colors", "")}>
      ...
      <div className="flex justify-between items-start mb-8">
        ...
        <button
          ...
          className={cn(
            "bg-[#00adb5] hover:opacity-90 text-white px-4 py-2 rounded-lg flex items-center gap-2 font-medium transition-all shadow-sm",
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr
                className={cn(
                  "bg-slate-50/50 border-b border-slate-100 text-slate-500 text-[11px] uppercase tracking-wider font-semibold",
                  "dark:bg-slate-800/30 dark:border-slate-800",
                )}
              >
              ...
              {distributors.map((item, index) => (
                ...
                >
                  ...
                  <td className="px-6 py-4 text-sm">
                    <div
                      className={cn("text-slate-600", "dark:text-slate-300")}
                    >
                      {item.email}
                    </div>
                  ...
                  <td className="px-6 py-4 text-center">
                    <span
                      className={cn(
                        "px-3 py-1 bg-cyan-50 text-[#00adb5] rounded-full text-[10px] font-bold",
                        "dark:bg-cyan-950/40",
                      )}
                    >
                      {item.regionName}
                    ...
                    <div className="flex justify-end gap-2 text-slate-400">
                      <button
                        ...
                        className={cn(
                          ...
                          "dark:hover:bg-slate-800 hover:text-[#00adb5]",
                        )}
                      >
                        <Edit2 size={18} />
                      </button>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
      ...
      >
        ...
        <div
          className={cn(
            "p-6 bg-slate-50 flex gap-3 border-t border-slate-100",
        ...
        >
          <button
            onClick={onClose}
            className={cn(
              "flex-1 px-4 py-2.5 border border-slate-200 bg-white text-slate-600 font-semibold rounded-xl hover:bg-slate-50",
              "dark:bg-slate-900 dark:border-slate-700 dark:text-slate-300",
            )}
          >
            Cancel
          </button>
          <button className="flex-1 px-4 py-2.5 bg-red-600 text-white font-semibold rounded-xl hover:bg-red-700 shadow-md shadow-red-500/20">
            Delete
          </button>
        </div>
      </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
      ...
      >
        <div
          className={cn(
            "p-6 flex justify-between items-start border-b border-slate-100",
        ...
        >
          ...
          <button className="flex-1 px-4 py-2.5 bg-[#00adb5] text-white font-semibold rounded-xl hover:opacity-90 shadow-md shadow-cyan-500/20">
...
function FormField({
  ...
  return (
    <div className="space-y-1.5">
      <label
        className={cn(
          "text-xs font-bold text-slate-700 flex items-center gap-1",
          "dark:text-slate-400",
        )}
      >
      ...
      <div className="relative">
        ...
        <input
          ...
          className={cn(
            "w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl focus:outline-none focus:border-[#00adb5] transition-all text-sm",
